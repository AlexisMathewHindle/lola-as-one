...
Current production focus: launch events and workshops first on `https://www.lotsoflovelyart.com`. Use [Events Production Launch Epic](./events-production-launch-epic.md) as the immediate execution plan for event discovery, bookings, Stripe, email confirmations, notifications, admin operations, waitlists, reminders, deployment, and go/no-go checks. Use [Events Pre-Launch Checklist](./events-pre-launch-checklist.md) and [Production Go-Live Checklist](./production-go-live-checklist.md) for final sign-off. Events Data And CMS Readiness and Public Discovery And Event Detail Flow are green as of 2026-05-14. Event Cart And Checkout has a green non-live automated audit. Stripe Payment And Webhook Proof is sandbox-complete and cleaned up as of 2026-05-19 for order, booking, attendee, capacity, success-page, Stripe event, email confirmation logs, duplicate webhook delivery, and restored proof capacity. Admin Booking Operations automated audit and staff browser proof are green as of 2026-05-19 after applying missing production schema, backfilling seven future attendee rows, and documenting the cancellation/refund runbook. Email Confirmations And Notifications operational readiness is deferred until missing email variables and sender/domain configuration are available. Google Analytics readiness is pending implementation/configuration. Stripe remains in sandbox/test mode; live Stripe cutover and live-mode proof are required before production launch.
...
- Stripe Payment And Webhook Proof is sandbox-complete on 2026-05-19. Order, order item, booking, attendee, success-page recovery, capacity consistency, Stripe event log, order-linked `order-confirmation`, `event-booking-confirmation`, `new-order-admin` sent logs, and duplicate webhook delivery all pass after fixing the webhook/booking-trigger capacity double-count and protected `send-email` gateway auth.
- Replay/idempotency proof passed on 2026-05-19 for duplicate Stripe event ID delivery and duplicate Checkout Session ID delivery. No duplicate order, order item, booking, attendee, capacity decrement, or email send was created.
- Sandbox proof cleanup passed on 2026-05-19: 9 proof bookings/orders were marked cancelled, 9 event spaces were restored across 3 event capacity rows, and capacity remained consistent.
- Email Confirmations And Notifications operational readiness is documented but deferred because missing email variables and sender/domain configuration block completion.
- Admin Booking Operations audit was added on 2026-05-19. Source/build checks are green after event orders were restored to the admin orders list and order links were added to booking/event details. Production schema gaps for check-in/allergy fields were applied, seven future bookings missing attendee rows were backfilled, and the automated audit now has 0 P0/P1 failures. The non-destructive staff browser proof is green across mobile and desktop/tablet viewports, and the cancellation/refund runbook is documented. A short smoke check on the actual venue device remains before the first live event.
